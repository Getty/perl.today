...
sub for_test {
  my ( $class, $tempdir ) = @_;
  my $pt = PT->new({ config => PT::Config->new(
...
sub update_notifications {
  my ( $self ) = @_;
  $self->pt->rs('Event')->result_class->meta->add_after_method_modifier('notify', sub {
    $self->next_step;
  });
...
sub add_comments {
  ...
  while (@comments) {
    ...
    my $comment = $self->pt->add_comment($context, $context_id, $user, $text);
    $self->next_step;
    $self->isa_ok($comment,'PT::DB::Result::Comment');
    if (scalar @sub_comments > 0) {
      $self->add_comments('PT::DB::Result::Comment',$comment->id, @sub_comments);
    }
  }
}
