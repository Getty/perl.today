...
sub _build_transport {
  my ( $self ) = @_;
  return Email::Sender::Transport::Test->new if $self->pt->config->mail_test;
  return Email::Sender::Transport::Sendmail->new unless $self->pt->config->smtp_host;
  my %smtp_args;
  $smtp_args{host} = $self->pt->config->smtp_host;
  $smtp_args{ssl} = $self->pt->config->smtp_ssl;
  $smtp_args{sasl_username} = $self->pt->config->smtp_sasl_username if $self->pt->config->smtp_sasl_username;
  $smtp_args{sasl_password} = $self->pt->config->smtp_sasl_password if $self->pt->config->smtp_sasl_password;
  return Email::Sender::Transport::SMTP->new({ %smtp_args });
}

sub mail {
  my ( $self, $to, $subject, $body, %extra_headers ) = @_;
  die __PACKAGE__."->mail needs to, subject, body" unless $body && $subject && $to;
