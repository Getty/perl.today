...
sub crud_base :Chained('base') :PathPart('') :CaptureArgs(0) {
  ...
  $c->stash->{object_name_attr} = 'name' unless defined $c->stash->{object_name_attr};
  $c->stash->{resultset_order} = { -asc => $c->stash->{object_key} } unless defined $c->stash->{resultset_order};
...
sub index :Chained('crud_base') :PathPart('') :Args(0) {
  ...
  $c->stash->{table} = $c->table(
    $c->stash->{resultset}->search_rs({},{
      order_by => $c->stash->{resultset_order},
    }),
    [$c->stash->{crud_controller},'index',@{$c->stash->{crud_captures}}],
    $c->stash->{columns},
    default_pagesize => 15,
...
sub add :Chained('crud_base') :Args(0) {
  my ( $self, $c ) = @_;
  $c->breadcrumb_add('Add',[$c->stash->{crud_controller},'add',@{$c->stash->{crud_captures}}]);
  $c->stash->{heading} = 'Add '.$c->stash->{object_name};
  $c->stash->{object} = $self->can('new_object')
    ? $self->new_object($c)
...
sub item :Chained('crud_base') :PathPart('') :CaptureArgs(1) {
  ...
  $c->stash->{object} = $c->stash->{resultset}->find({
    $c->stash->{object_key}, $key
  });
  my $obj_name_attr = $c->stash->{object_name_attr};
  $c->stash->{heading} = $c->stash->{object_title}.' <strong>'.$c->stash->{object}->$obj_name_attr.'</strong>';
...
sub edit :Chained('item') :Args(0) {
  my ( $self, $c ) = @_;
  my $obj_key = $c->stash->{object_key};
  $c->breadcrumb_add('Edit',[$c->stash->{crud_controller},'edit',@{$c->stash->{crud_captures}},$c->stash->{object}->$obj_key]);
  return $self->form($c);
}
...
sub delete :Chained('item') :Args(0) {
  ...
  my $obj_key = $c->stash->{object_key};
  $c->breadcrumb_add('Delete',[$c->stash->{crud_controller},'delete',@{$c->stash->{crud_captures}},$c->stash->{object}->$obj_key]);
  $self->push_crud_template($c);
}
...
sub form {
  ...
  $c->response->redirect($c->stash->{crud_url}->());
  my $obj_name_attr = $c->stash->{object_name_attr};
  $c->add_alert('Saved '.$c->stash->{object_name}.' <strong>'.$c->stash->{object}->$obj_name_attr.'</strong>', 'success');
  if ($self->can('success')) {
