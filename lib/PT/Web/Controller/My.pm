...
sub requestvalidate :Chained('logged_in') :Args(1) {
  ...
  my $user_email = $c->user->search_related('user_emails',{
    email => $email,
  })->first;
...
sub emailvalidate :Chained('base') :Args(2) {
  ...
  my @emails = $user->search_related('user_emails',{
    token => $token,
  })->all;
  ...
  $user->search_related('user_emails',{
    token => $token,
...
sub resetpw :Chained('logged_out') :Args(2) {
  ...
  if (!defined $c->req->params->{password} or length($c->req->params->{password}) < 5) {
    $c->stash->{password_too_short} = 1;
    $error = 1;
  }
...
sub nickname :Chained('logged_in') :Args(0) {
  ...
  if ($c->req->params->{new_nickname} ne $c->user->nickname) {
    ...
    $c->user->update;
    $c->response->redirect($c->chained_uri('My','account',{ nickname_changed => 1 }));
    return $c->detach;
  }
