package PT::DB::ResultSet::Comment;
...
sub grouped_by_context {
    my ($self) = @_;
    my $comment_context_rs =
        $self->schema->resultset('Comment::Context')->search_rs(
        {},
    ...
    $self->search_rs(
        ...
        {   '+columns' => {
                comments_count =>
                    $self->schema->resultset('Comment')->search_rs(
                    {   'comments_count.context' =>
                            { -ident => $self->me . 'context' },
                        'comments_count.context_id' =>
                            { -ident => $self->me . 'context_id' },
                    },
