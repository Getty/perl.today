...
has_many 'user_notifications', 'PT::DB::Result::UserNotification',
    'users_id', { cascade_delete => 1, };
has_many 'user_roles', 'PT::DB::Result::UserRole', 'users_id',
    { cascade_delete => 1, };
has_many 'user_notification_matrixes',
    'PT::DB::Result::UserNotificationMatrix', 'users_id',
    { cascade_delete => 0, };
...
sub add_email {
    ...
    my $user_email =
        $self->schema->resultset('UserEmail')->search( { email => $email, } )
        ->first;
    if ($user_email) {
...
sub remove_email {
    my ( $self, $email ) = @_;
    return $self->search_related( 'user_emails', { email => $email, } )
        ->delete;
}
...
sub email {
    my ($self) = @_;
    my $primary = $self->search_related(
        'user_emails',
        {   validated => 1,
            primary   => 1,
        }
    ...
    my $secondary = $self->search_related(
        'user_emails',
        {   validated => 1,
            primary   => 0,
        }
...
sub _build_user_notification_group_values {
        ...
        )
    {
        $user_notification_group_values{ $_->user_notification_group->type }
            = {}
            unless defined
            $user_notification_group_values{ $_->user_notification_group
                ->type };
        my $context_id_key =
            $_->user_notification_group->with_context_id ? '*' : '';
        $user_notification_group_values{ $_->user_notification_group->type }
            ->{$context_id_key} = { cycle => $_->cycle, xmpp => $_->xmpp };
    }
...
sub add_context_notification {
    ...
    if ( $group_info->{cycle} ) {
        my @user_notification_groups =
            $self->schema->resultset('UserNotificationGroup')->search(
            {   context         => $context_obj->context_name,
                with_context_id => 1,
                type            => $type,
            }
            )->all;
        die "Several notification groups found, cant be..."
            if scalar @user_notification_groups > 1;
        die "No notification group found!"
            if scalar @user_notification_groups < 1;
        my $user_notification_group = $user_notification_groups[0];
        return $self->update_or_create_related(
            'user_notifications',
            ...
            {   key =>
                    'user_notification_user_notification_group_id_context_id_users_id',
            }
        );
    }
}
