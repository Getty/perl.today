...
belongs_to 'user', 'PT::DB::Result::User', 'users_id', { join_type => 'left' };

has_many 'event_notifications', 'PT::DB::Result::EventNotification', 'event_id', {
  cascade_delete => 1,
};
...
sub notify {
  ...
  $self->schema->txn_do(sub {
    ...
    my @user_notifications = $self->schema->resultset('UserNotification')->search({
      -or => \@queries,
    },{
    ...
    if (@user_notifications) {
      ...
      for my $user_notification (@user_notifications) {
        ...
        my $event_notification_group = $self->schema->resultset('EventNotificationGroup')->find_or_create({
