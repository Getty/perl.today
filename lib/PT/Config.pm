...
has_conf templatedir => PT_TEMPLATEDIR => sub {
    dir( Catalyst::Utils::home('PT'), 'templates' )
        ->resolve->absolute->stringify;
};
...
sub xslate_cachedir {
    my ($self) = @_;
    my $dir =
        defined $ENV{'PT_CACHEDIR_XSLATE'}
        ? $ENV{'PT_CACHEDIR_XSLATE'}
        : $self->cachedir() . '/xslate/';
    make_path($dir) if !-d $dir;
    return File::Spec->rel2abs($dir);
}
