...
sub deploy_fresh {
	...
	die "ARE YOU INSANE????? KILLING LIVE???? GO DING-DING-DING YOURSELF!!!" if $self->is_live;
...
sub as {
	my ( $self, $user, $code ) = @_;
	die "as need user or undef" unless !defined $user || $user->isa('PT::DB::Result::User');
...
sub find_user {
	...
	die "Unknown method ".$method." for fetching account!" unless $self->db->resultset('User')->can($func);
...
sub user_counts {
	...
	return $self->cache->get('pt_user_counts') if defined $self->cache->get('pt_user_counts');
